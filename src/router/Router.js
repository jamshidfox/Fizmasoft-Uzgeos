// ** React Imports
import { useEffect, Suspense, useContext, lazy } from "react"
...
import { handleSocketAdd } from "../redux/actions/socket/"
...
const Router = ({ history }) => {
  ...
  dispatch(handleSocketAdd(socket))
      ...
      socket.on("notifications", (data) => {
        if (user && user.group) {
          ...
          if (data.notification.group_id.includes(userId)) {
            dispatch(handleAlarmStatus(true, data))
          }
        }
      })
    }
    ...
    return Object.keys(Layouts).map((layout, index) => {
      ...
      return (
        <Route path={LayoutPaths} key={index}>
          <LayoutTag routerProps={routerProps} layout={layout} setLayout={setLayout} transition={transition} setTransition={setTransition} currentActiveItem={currentActiveItem}>
            <Switch>
